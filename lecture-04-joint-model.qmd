...
dat_surv<-read.csv("egfr_joint_baseline.csv")
...
**Overall**
- In this joint model, eGFR declines over time and is strongly influenced by donor type, donor age, and recipient sex.  
...
Figure B shows **Dynamic joint-model predictions** for a randomly selected transplant recipient.
The left panel shows the observed longitudinal eGFR measurements (solid points and lines) together with the subject-specific fitted trajectory from the linear mixed model (dashed line). The right panel displays the corresponding dynamic predicted graft survival probability, updated sequentially as new eGFR measurements become available. Shaded areas represent 95% credible intervals.
...
# data
dat_long <- read.csv("egfr_joint_data.csv")          # long data with repeated eGFR
dat_surv <- read.csv("egfr_joint_baseline.csv")      # wide / baseline survival data
